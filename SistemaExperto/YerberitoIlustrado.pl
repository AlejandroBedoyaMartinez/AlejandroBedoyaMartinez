...
:- consult('C:/SistemaExperto/origen.pl').
:- consult('C:/SistemaExperto/nombreCien.pl').
:- consult('C:/SistemaExperto/preparacion.pl').
:- consult('C:/SistemaExperto/tratamiento.pl').
...
consultar_origen :-
    new(DialogoConsulta, dialog('Consultar origen')),
    send(DialogoConsulta, append, new(TextoConsulta, text_item('Escribe el nombre de la planta:'))),
    send(DialogoConsulta, append, button('Aceptar', 
        message(@prolog, obtener_consulta, origen, DialogoConsulta, TextoConsulta))),
    send(DialogoConsulta, append, button('Cancelar', 
        message(DialogoConsulta, destroy))),
    send(DialogoConsulta, open).

consultar_nombre_cientifico :-
    new(DialogoConsulta, dialog('Consultar nombre científico')),
    send(DialogoConsulta, append, new(TextoConsulta, text_item('Escribe el nombre común de la planta:'))),
    send(DialogoConsulta, append, button('Aceptar', 
        message(@prolog, obtener_consulta, nombreCientifico, DialogoConsulta, TextoConsulta))),
    send(DialogoConsulta, append, button('Cancelar', 
        message(DialogoConsulta, destroy))),
    send(DialogoConsulta, open).

consultar_preparacion :-
    new(DialogoConsulta, dialog('Consultar preparación')),
    send(DialogoConsulta, append, new(TextoConsulta, text_item('Escribe el nombre de la planta:'))),
    send(DialogoConsulta, append, button('Aceptar', 
        message(@prolog, obtener_consulta, preparacion, DialogoConsulta, TextoConsulta))),
    send(DialogoConsulta, append, button('Cancelar', 
        message(DialogoConsulta, destroy))),
    send(DialogoConsulta, open).

consultar_tratamiento :-
    new(DialogoConsulta, dialog('Consultar tratamiento')),
    send(DialogoConsulta, append, new(TextoConsulta, text_item('Escribe el nombre de la planta:'))),
    send(DialogoConsulta, append, button('Aceptar', 
        message(@prolog, obtener_consulta, tratamiento, DialogoConsulta, TextoConsulta))),
    send(DialogoConsulta, append, button('Cancelar', 
        message(DialogoConsulta, destroy))),
    send(DialogoConsulta, open).

obtener_consulta(Tipo, DialogoConsulta, TextoConsulta) :-
    get(TextoConsulta, selection, Planta),
    consulta(Tipo, Planta, Resultados),
    mostrar_resultado(Resultados),
    send(DialogoConsulta, destroy).

consulta(origen, Planta, Resultados) :-
    findall(Resultado, origen(Planta, Resultado), Resultados).

consulta(nombreCientifico, Planta, Resultados) :-
    findall(Resultado, nombreCientifico(Planta, Resultado), Resultados).

consulta(preparacion, Planta, Resultados) :-
    findall(Resultado, preparacion(Planta, Resultado), Resultados).

consulta(tratamiento, Planta, Resultados) :-
    findall(Resultado, tratamiento(Planta, Resultado), Resultados).

mostrar_resultado(Resultados) :-
    new(Dialogo, dialog('Resultado de la consulta')),
    new(TextoResultado, text), 
    forall(member(Resultado, Resultados),
           send(TextoResultado, append, Resultado)), 
    send(Dialogo, append, TextoResultado),  
    send(Dialogo, append, button('Cerrar', 
        message(Dialogo, destroy))),  
    send(Dialogo, open). 
...
iniciar :-
    ...
    new(Menu2, popup('Conocimientos: ')),
    send(MenuBar, append, Menu2),
    send_list(Menu2, append,
              [ menu_item('Origenes', message(@prolog, consultar_origen)),
                menu_item('Nombre cientifico', message(@prolog, consultar_nombre_cientifico)),
                menu_item('Preparacion', message(@prolog, consultar_preparacion)),
                menu_item('Tratamiento', message(@prolog, consultar_tratamiento))
              ]),        
